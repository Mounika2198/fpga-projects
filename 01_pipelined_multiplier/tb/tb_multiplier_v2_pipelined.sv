...
// Module  : Testbench for Multiplier V2
//
// Description:
//   Self-checking testbench for the pipelined
//   partial-product multiplier.
//
//==================================================

module tb_multiplier_v2_pipelined;
...
logic [(2*WIDTH)-1:0] expected_stage3;

logic expected_valid_stage1;
logic expected_valid_stage2;
logic expected_valid_stage3;
...
always @(negedge clk) begin
    if (rst_n) begin
        ...
        if (out_valid !== expected_valid_stage3) begin
            ...
            $display(
                ...
                $time,
                out_valid,
                expected_valid_stage3
            );
        end

        if (expected_valid_stage3) begin

            check_count = check_count + 1;

            if (product !== expected_stage3) begin
                error_count = error_count + 1;
...
initial begin
    ...
    if (error_count == 0) begin
        $display(
            "V2 TEST PASSED : %0d checks, 0 errors",
            check_count
        );
    end
    else begin
        $display(
            "V2 TEST FAILED : %0d checks, %0d errors",
            check_count,
            error_count
        );
    end
